...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode='const', fused_modconv=True, gain=1):
        ...
        if self.use_noise and noise_mode == 'random':
            noise = torch.randn([x.shape[0], self.resolution, self.resolution], device=x.device) * self.noise_strength
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, layer_in=None, start_layer=0, end_layer=16, **block_kwargs):
        block_ws = []
        ...
        img = None
        index = start_layer
        for res, cur_ws in zip(self.block_resolutions[start_layer:], block_ws[start_layer:]):
            block = getattr(self, f'b{res}')
